...
void  mergeSort(myType* array,bool(*func)(myType,myType),int n)
{
	if (n==1)
		return ;
        else { 
		...
		mergeSort(leftSortedArray,func,r);//sorting individual halfs
                mergeSort(rightSortedArray,func,n-r);
		for (i=0,j=0,k=0;i<r && j <n-r;)//merging two halfs
                {
  			if(func(leftSortedArray[i],rightSortedArray[j]))
			{
